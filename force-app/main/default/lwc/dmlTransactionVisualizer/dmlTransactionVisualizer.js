...
export default class DmlTransactionVisualizer extends LightningElement {
  ...
  uploadedParserMessageHandler = (event) =>
    this.handleUploadedParserMessage(event);
  ...
  connectedCallback() {
    this.uploadedParserFrameOrigin = new URL(
      dmlLogParserFrameUrl,
  ...
  get uploadStatusClass() {
    const state = ["reading", "parsing", "ready", "error"].includes(
      this.uploadState
    )
      ? this.uploadState
      : "idle";
    ...
    if (nextTab === "tree") {
      ...
      if (hasActions) {
        ...
        this.treeReadyTimeoutId = setTimeout(() => {
          if (
            this.activeInspectorTab === "tree" &&
            this.activeTreeRenderToken === renderToken
          ) {
            this.tabLoading = false;
            this.treeLoading = false;
  ...
  handleTreeReady(event) {
    if (
      event.detail?.renderToken !== this.activeTreeRenderToken ||
      this.activeInspectorTab !== "tree"
    ) {
      return;
    }
  ...
  get queueEmptyTitle() {
    ...
    if (
      this.queueStats.internalDmlCount > 0 &&
      this.queueStats.confirmedDmlCount === this.queueStats.internalDmlCount
    ) {
  ...
  get fieldChangesGroups() {
    if (
      Array.isArray(this.detail.fieldChangesGroups) &&
      this.detail.fieldChangesGroups.length > 0
    ) {
    ...
    return [
      {
        ...
        recordUrl:
          this.selectedRecordUrl !== "#" ? this.selectedRecordUrl : null,
  ...
  get fieldGroupPageCount() {
    return Math.max(
      1,
      Math.ceil(this.fieldChangesGroups.length / FIELD_GROUP_PAGE_SIZE)
    );
  }
  ...
  get selectedFieldChangesGroup() {
    ...
    return (
      pageGroups.find(
        (group) => group.groupKey === this.selectedFieldGroupKey
      ) || pageGroups[0]
    );
  }
  ...
  previousFieldGroupPage() {
    if (this.fieldGroupPage > 1) {
      ...
      this.selectedFieldGroupKey =
        this.fieldChangesGroups[
          (this.fieldGroupPage - 1) * FIELD_GROUP_PAGE_SIZE
        ]?.groupKey;
    }
  }
  ...
  get selectedCard() {
    ...
    this.selectedCardCacheDetail = this.detail;
    this.selectedCardCacheId = this.selectedId;
    this.selectedCardCacheValue =
      this.transactions.find(
        (item) => item.Transaction_Id__c === this.selectedId
      ) || this.detail?.log;
    return this.selectedCardCacheValue;
  }
  ...
  get isLogTruncated() {
    return Boolean(
      this.detail?.executionTree?.isTruncated || this.detail?.log?.isTruncated
    );
  }
  ...
  get detailTimeLabel() {
    const rawTime =
      this.selectedCard?.timestampStr ||
      this.selectedCard?.Start_Time__c ||
      this.selectedCard?.StartTime ||
      this.detail.log?.Start_Time__c ||
      this.detail.log?.StartTime ||
      this.detail.log?.startTime;
  ...
  get selectedCardObject() {
    return (
      this.selectedCard?.Object_API_Name__c ||
      this.detail?.log?.Object_API_Name__c ||
      "Unknown"
    );
  }
  ...
  get selectedCardOperation() {
    return (
      this.selectedCard?.DML_Type__c || this.detail?.log?.DML_Type__c || "DML"
    );
  }
  ...
  get selectedCardRowCountLabel() {
    const count =
      this.selectedCard?.rowCount ?? this.detail?.log?.rowCount ?? 1;
  ...
  get selectedRecordId() {
    ...
    if (scopedLog && dmlScope) {
      const scopedGroup = (this.detail?.fieldChangesGroups || []).find(
        (group) => group.objectName === objName && group.isRealId
      );
  ...
  get selectedCardUser() {
    return (
      this.selectedCard?.Triggered_By__c ||
      this.detail?.log?.Triggered_By__c ||
      "Current User"
    );
  }
  ...
  get selectedCardDuration() {
    return (
      this.selectedCard?.durationLabel ||
      this.detailDurationLabel ||
      `${this.selectedCard?.durationMs || 0} ms`
    );
  }
  ...
  handleCopyRecordId() {
    ...
    this.showToast(
      "Copied",
      `Record ID (${this.selectedRecordId}) copied to clipboard.`,
      "success"
    );
  }

  get executionHealthClass() {
    return this.hasHealthErrors
      ? "important-events has-error"
      : "important-events";
  }

  get failedHealthEvents() {
    return this.importantEvents.filter(
      (eventItem) => eventItem.status === "Failed"
    );
  }
  ...
  get importantEvents() {
    ...
    const hasValidationError =
      /validation|FIELD_CUSTOM_VALIDATION_EXCEPTION|REQUIRED_FIELD_MISSING|required fields are missing/i.test(
        errorText
      );
    ...
    const flowSteps = this.steps.filter(
      (step) =>
        step.Step_Type__c === "Flow" || /^Flow:/i.test(step.Step_Name__c || "")
    );
    const failedFlowStep = flowSteps.find(
      (step) => step.Status__c === "Failed"
    );
    const hasFlowError =
      Boolean(failedFlowStep) ||
      /flow|FLOW_|CANNOT_EXECUTE_FLOW_TRIGGER|FLOW_ELEMENT_ERROR|process builder/i.test(
        errorText
      );
    const failedSteps = this.steps.filter(
      (step) => step.Status__c === "Failed"
    );
    ...
    if (isTransactionFailed && rootErrorText) {
      events.push(
        this.buildEvent("rootError", "Error", rootErrorText, "Failed", {
          longDetail:
            "A failed event was captured for this transaction. Use this card first to identify the root error regardless of whether it came from Apex, Flow, validation, DML, or trigger logic.",
          metrics: [
            {
              label: "Object",
              value: this.detail.log?.Object_API_Name__c || "-"
            },
            { label: "DML Type", value: this.detail.log?.DML_Type__c || "-" },
            {
              label: "Failed Step",
              value: firstFailedStep?.Step_Name__c || "-"
            },
            { label: "Step Type", value: firstFailedStep?.Step_Type__c || "-" },
            {
              label: "Transaction",
              value: this.detail.log?.Transaction_Id__c || "-"
            }
          ],
          actions: [
    ...
    if (hasValidationError) {
      events.push(
        this.buildEvent("validation", "Validation", errorText, "Failed", {
          longDetail:
            "A validation or required-field error was found in the transaction error message.",
          metrics: [
            { label: "Validation Status", value: "Failed" },
            { label: "Failed Steps", value: String(failedSteps.length) },
            {
              label: "Object",
              value: this.detail.log?.Object_API_Name__c || "Unknown"
            }
          ],
          actions: [
    ...
    const extractFlowItems = (nodes) => {
      (nodes || []).forEach((act) => {
        if (
          act.type === "FLOW_START_INTERVIEW_BEGIN" ||
          (act.type === "CODE_UNIT_STARTED" &&
            /^Flow:/i.test(act.name || "") &&
            !act.children?.some((c) => c.type === "FLOW_START_INTERVIEW_BEGIN"))
        ) {
          const elements = (act.children || [])
            .filter(
              (c) =>
                c.type === "FLOW_ELEMENT_BEGIN" || c.label === "Flow Element"
            )
            .map((c) => c.name);
          const elementSummary =
            elements.length > 0 ? ` (Element: ${elements.join(", ")})` : "";
    ...
    const flowNamesList = Array.from(
      new Set(flowOverviewItems.map((item) => item.name))
    ).filter(Boolean);
    ...
    const soqlTargetsList = Array.from(
      new Set(
        soqlOverviewItems.map((item) => {
    ...
    const soqlTargetsSummary =
      soqlTargetsList.length > 0 ? ` (${soqlTargetsList.join(", ")})` : "";
    ...
    const soslTargetsList = Array.from(
      new Set(
        soslOverviewItems.map((item) => {
          const text = item.name || "";
    ...
    const soslTargetsSummary =
      soslTargetsList.length > 0 ? ` (${soslTargetsList.join(", ")})` : "";
        ...
        this.buildEvent(
          ...
          hasFlowError ? "Failed" : "Success",
          {
            longDetail: hasFlowError
              ...
              : isDebugLogCard
                ? "Flow-related debug events were captured in the selected DML block."
                : "Flow logger step(s) were captured for this transaction.",
            metrics: [
              {
                label: "Flows Captured",
                value: String(flowOverviewItems.length || flowSteps.length)
              },
              {
                label: "Failed Flow Step",
                value: failedFlowStep?.Step_Name__c || "-"
              },
              {
                label: "Capture Mode",
                value: isDebugLogCard
                  ? "Debug Log Parse"
                  : "Explicit Flow Logger"
              }
            ],
            ...
            errorMessage: hasFlowError
              ? failedFlowStep?.Error_Message__c || errorText
              : ""
          }
        )
      );
    ...
    if (
      ...
      events.push(
        this.buildEvent(
          "exceptions",
          "Exceptions",
          errorText || "Apex Exception captured",
          "Failed",
          {
            longDetail:
              "An exception was captured for this transaction. Failed steps are listed below.",
            metrics: [
              { label: "Failed Steps", value: String(failedSteps.length) },
              {
                label: "Transaction Status",
                value: this.detail.log?.Status__c || "-"
              },
              {
                label: "Error Source",
                value: firstFailedStep?.Error_Class__c || "-"
              },
              {
                label: "Error Line",
                value: firstFailedStep?.Error_Line__c
                  ? String(firstFailedStep.Error_Line__c)
                  : "-"
              }
            ],
            actions: [
              "Open the failed step to inspect stack trace and line number."
            ],
            relatedSteps: failedSteps,
            errorMessage: errorText
          }
    ...
    if (totalSoql > 0 || hasSoqlLimitError || soqlOverviewItems.length > 0) {
      events.push(
        this.buildEvent(
          ...
          "SOQL",
          soqlCardSubtitle,
          hasSoqlLimitError ? "Failed" : "Success",
          {
            ...
            metrics: [
              {
                label: "Total Queries",
                value: String(soqlOverviewItems.length || totalSoql)
              },
            ...
            relatedSteps: this.steps.filter(
              (step) => step.SOQL_Queries_Used__c > 0 || hasSoqlLimitError
            )
          }
        )
      );
    ...
    if (soslOverviewItems.length > 0) {
      events.push(
        this.buildEvent("sosl", "SOSL", soslCardSubtitle, "Success", {
          longDetail:
            "SOSL text search statements executed in this transaction.",
          metrics: [
    ...
    this.importantEventsCacheSelectedId = this.selectedId;
    this.importantEventsCacheHealthKey = this.selectedHealthEventKey;
    this.importantEventsCacheValue = events.sort(
  ...
  async loadTransactions(options = {}) {
    ...
    const isManualRefresh = Boolean(
      options?.target || loadOptions.forceReload === true
    );
    const shouldPreserveSelection = loadOptions.preserveSelection === true;
    const shouldClearSelectionWhenHidden =
      loadOptions.clearSelectionWhenHidden === true;
    ...
    try {
      ...
      const transactionRows = businessRows.filter(
        (row) => row && this.matchesFilters(row)
      );
      this.syncObjectOptions(businessRows);
      const selectedRow = transactionRows.find(
        (row) => row.Transaction_Id__c === this.selectedId
      );
      ...
      this.syncObjectOptions([]);
      this.debugLogError = this.getRemoteErrorMessage(
        error,
  ...
  async scanPendingLogs(pendingLogs, sessionId, signal) {
    ...
    try {
      ...
      if (sessionId === this.scanSessionId && !signal?.aborted) {
        this.debugLogError = this.getRemoteErrorMessage(
          error,
          "Scanning stopped before all recent logs could be checked. Please click Refresh and try again."
        );
      }
    } finally {
      if (sessionId === this.scanSessionId && !signal?.aborted) {
    ...
    try {
      if (Number(log.LogLength) > MAX_FETCHABLE_LOG_BYTES) {
        if (sessionId === this.scanSessionId) {
          ...
          this.debugLogError =
            "Some recent logs exceed the interactive parsing limit and were skipped.";
        }
        return;
      }
      ...
      if (rawLog.length > MAX_FETCHABLE_LOG_BYTES) {
        ...
        throw new Error(
          "This Apex debug log exceeds the interactive parsing limit and could not be analyzed."
        );
      }
      ...
      const parsedResult = await extractDmlCardResult(tree, {
      ...
      if (
        ...
        const newRows = summaries.map((summary, index) =>
          this.createDmlSummaryRow({
            ...log,
            DmlIndex: Number.isFinite(Number(summary.dmlIndex))
              ? Number(summary.dmlIndex)
              : index,
      ...
      const message = this.getRemoteErrorMessage(
        error,
        "Unable to scan this debug log."
      );
      if (sessionId === this.scanSessionId) {
  ...
  addScannedRows(rows, sessionId) {
    ...
    const existingIds = new Set(
      (this.rawTransactionRows || []).map((row) => row.Transaction_Id__c)
    );
    const merged = [
      ...(this.rawTransactionRows || []),
      ...rows.filter((row) => !existingIds.has(row.Transaction_Id__c))
    ];
  ...
  saveQueueSnapshot() {
    const rows = (this.rawTransactionRows || [])
      .filter((row) => !row.UploadedFile)
      .map((row) => {
  ...
  restoreQueueSnapshot() {
    ...
    this.rawTransactionRows = (queueSnapshot.rows || []).map((row) => ({
      ...row
    }));
  ...
  createDmlSummaryRow(log) {
    ...
    const rowCount =
      log.DmlRows === null ||
      log.DmlRows === undefined ||
      !Number.isFinite(Number(log.DmlRows))
        ? 1
        : Number(log.DmlRows);
    const dmlIndex = Number.isFinite(Number(log.DmlIndex))
      ? Number(log.DmlIndex)
      : 0;
    ...
    const endNanos =
      ...
      Number.isFinite(Number(endValue))
        ...
        : null;
    const startByte =
      log.DmlStartByte !== null &&
      log.DmlStartByte !== undefined &&
      Number.isFinite(Number(log.DmlStartByte))
        ? Number(log.DmlStartByte)
        : null;
    const endByte =
      log.DmlEndByte !== null &&
      log.DmlEndByte !== undefined &&
      Number.isFinite(Number(log.DmlEndByte))
        ? Number(log.DmlEndByte)
        : null;
    const durationMs =
      Number.isFinite(startNanos) && Number.isFinite(endNanos)
    ...
    return {
      ...
      Status__c:
        (log.DmlStatus || log.Status) === "Failed" ? "Failed" : "Success",
      ...
      dmlScope: {
        startNanos,
        endNanos,
        startLine: Number.isFinite(Number(log.DmlStartLine))
          ? Number(log.DmlStartLine)
          : null,
        endLine: Number.isFinite(Number(log.DmlEndLine))
          ? Number(log.DmlEndLine)
          : null,
        objectName,
        operation,
        rowCount,
        startByte,
        endByte,
        contextEvents: Array.isArray(log.DmlContextEvents)
          ? log.DmlContextEvents
          : []
      },
      actions: [],
      hasActions: false,
  ...
  formatQueueRows(rows) {
    return (rows || [])
      .filter((row) => row && this.matchesFilters(row))
      .map((row) => {
        const isSelected =
          row.Transaction_Id__c === this.selectedId ||
  ...
  getOperationBadgeClass(op) {
    ...
    if (lower === "upsert" || lower === "merge")
      return "operation-pill op-upsert";
  ...
  handleUploadedParserFrameError() {
    ...
    this.rejectUploadedParserFrame(
      new Error(
        "The local log parser could not be loaded. Please refresh the page and try again."
      )
    );
  }
  ...
  handleUploadedParserMessage(event) {
    const frame =
      this.uploadedParserFrame || this.template.querySelector(".parser-frame");
    ...
    if (message.type === "READY") {
      ...
      resolvers.forEach((resolver) =>
        resolver.resolve(this.uploadedParserFrame)
      );
      return;
    }
    ...
    if (message.type === "PROGRESS") {
      if (message.phase === "indexing" && this.uploadState === "parsing") {
        this.uploadProgress = Math.min(
          85,
          Math.round(Number(message.progress || 0) * 85)
        );
      }
      return;
    }
    if (message.type === "RESULT") {
      ...
      pending.settleReject(
        new Error(
          message.message ||
            "The local log parser failed while processing the file."
        )
      );
    }
  }
  ...
  waitForUploadedParserFrame(signal = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      timeoutId = setTimeout(() => {
        ...
        resolver.reject(
          new Error(
            "The local log parser did not start in time. Please refresh the page and try again."
          )
        );
  ...
  runUploadedWorkerRequest(type, payload = {}, signal = null) {
    ...
    if (this.uploadedWorkerPending) {
      return Promise.reject(
        new Error(
          "Another file is still being parsed. Please wait for it to finish or choose a new file."
        )
      );
    }
    const requestId = ++this.uploadedWorkerRequestId;
    return this.waitForUploadedParserFrame(signal).then(
      (frame) =>
        new Promise((resolve, reject) => {
          ...
          const settleResolve = (value) => {
            ...
            settled = true;
            cleanup();
            if (this.uploadedWorkerPending?.requestId === requestId)
          ...
          timeoutId = setTimeout(() => {
            ...
            settleReject(
              new Error(
                "Local log parsing took too long. Try a smaller file or a simpler debug log."
              )
            );
          }, UPLOADED_PARSER_REQUEST_TIMEOUT_MS);
          try {
            const messagePayload =
              type === "PARSE_SCOPE" ? { scope: payload.scope } : payload;
            frame.contentWindow.postMessage(
              {
                channel: "DML_LOG_PARSER_FRAME_V1",
                type,
  ...
  destroyUploadedWorker() {
    ...
    if (
      this.uploadedParserFrame?.contentWindow &&
      this.uploadedParserFrameReady
    ) {
      try {
  ...
  async handleFileUpload(event) {
    ...
    try {
      if (file.size > MAX_UPLOAD_BYTES) {
        throw new Error(
          "This file is larger than the 60 MB interactive parsing limit."
        );
      }
      if (sessionId !== this.uploadedParseSession || signal.aborted) return;
      this.uploadState = "parsing";
      const indexResult = await this.runUploadedWorkerRequest(
        "INDEX_FILE",
        { file },
        signal
      );
      ...
      const orgRows = (this.rawTransactionRows || []).filter(
      ...
      this.queueStats = {
        ...
        confirmedDmlCount:
          (this.queueStats.confirmedDmlCount || 0) + cards.length
      };
      ...
      if (
        error?.name === "AbortError" ||
        sessionId !== this.uploadedParseSession
      )
        return;
      this.uploadState = "error";
      this.uploadMessage = this.getParserErrorMessage(
        error,
  ...
  async handleSelect(event) {
    ...
    const row = (this.rawTransactionRows || this.transactions).find(
      (item) => item.Transaction_Id__c === transactionId
    );
    if (!row) return;
    ...
    try {
      ...
      this.transactions = this.formatQueueRows(
        this.rawTransactionRows || this.transactions
      );
    } catch (error) {
      if (error?.name === "AbortError") return;
      const message = this.getRemoteErrorMessage(
        error,
        "We could not load this transaction's details. Please select it again."
      );
  ...
  handleHealthEventToggle(event) {
    ...
    this.selectedHealthEventKey =
      this.selectedHealthEventKey === eventKey ? null : eventKey;
  }
  ...
  async getUploadedScopeResult(row, signal) {
    if (!this.uploadedFile) {
      throw new Error(
        "The uploaded file is no longer available. Please choose the file again."
      );
    }
    ...
    const workerResult = await this.runUploadedWorkerRequest(
      "PARSE_SCOPE",
      {
    ...
    if (workerResult?.tooLarge) {
      throw new Error(
        workerResult.message ||
          "This DML scope is too large to display interactively."
      );
    }
    const result = {
    ...
    while (this.uploadedScopeCache.size >= MAX_UPLOADED_SCOPE_CACHE_ENTRIES) {
      this.uploadedScopeCache.delete(
        this.uploadedScopeCache.keys().next().value
      );
    }
  ...
  async loadDebugTransactionDetail(row, preserveStepSelection = false) {
    ...
    if (cachedDetail) {
      ...
      this.fieldGroupPage = 1;
      this.selectedFieldGroupKey =
        cachedDetail.fieldChangesGroups?.[0]?.groupKey;
      const cachedSteps = cachedDetail.steps || [];
      const cachedPriorStep = cachedSteps.find(
        (step) => step.Id === priorStepId
      );
      const cachedFailedStep = cachedSteps.find(
        (step) => step.Status__c === "Failed"
      );
      this.selectedStepId =
        cachedPriorStep?.Id || cachedFailedStep?.Id || cachedSteps[0]?.Id;
    ...
    if (!row.UploadedFile && this.debugLogTrees.has(row.DebugLogId)) {
      ...
      executionTree = await buildExecutionTreeAsync(rawLog || "", {
        chunkSize: 1000,
        signal
      });
    ...
    if (!row.UploadedFile && this.debugLogCardResults.has(row.DebugLogId)) {
      ...
      treeResult = await extractDmlCardResult(executionTree, {
        includeInferredUiSaves: false,
        signal
      });
    ...
    const matchingCard = (treeResult.cards || []).find((card) => {
      const sameScope =
        Number.isFinite(Number(row.dmlScope?.startNanos)) &&
    ...
    const extractedResult = row.UploadedFile
      ...
      : await extractFieldChangesFromDebugLog(rawLog || "", detailScope, {
          signal
        });
    const parsedDetail = {
      log: row,
  ...
  setDebugLogCardResult(logId, result) {
    ...
    if (this.debugLogCardResults.has(logId))
      this.debugLogCardResults.delete(logId);
    while (this.debugLogCardResults.size >= MAX_RAW_LOG_CACHE_ENTRIES) {
      this.debugLogCardResults.delete(
        this.debugLogCardResults.keys().next().value
      );
    }
  ...
  sumStepField(fieldName) {
    return (this.steps || []).reduce(
      (total, step) => total + this.toNumber(step[fieldName]),
      0
    );
  }
  ...
  maxStepField(fieldName) {
    return (this.steps || []).reduce(
      (maximum, step) => Math.max(maximum, this.toNumber(step[fieldName])),
      0
    );
  }
    ...
    const relatedSteps = (options.relatedSteps || []).map((step) => ({
      ...
      healthRowClass:
        step.Status__c === "Failed"
          ? "health-step-row failed"
          : "health-step-row"
    }));
    const metrics = [...(options.metrics || [])];
    const overviewItems = options.overviewItems || [];
    const failedRelatedStep = relatedSteps.find(
      (step) => step.Status__c === "Failed"
    );
    const eventErrorMessage =
      options.errorMessage || failedRelatedStep?.Error_Message__c || "";
    if (
      isFailed &&
      failedRelatedStep?.Error_Class__c &&
      !metrics.some((metric) => metric.label === "Error Source")
    ) {
      metrics.push({
        label: "Error Source",
        value: failedRelatedStep.Error_Class__c
      });
    }
    if (
      isFailed &&
      failedRelatedStep?.Error_Line__c &&
      !metrics.some((metric) => metric.label === "Line Number")
    ) {
      metrics.push({
        label: "Line Number",
        value: String(failedRelatedStep.Error_Line__c)
      });
    }
    return {
      key,
      ...
      detailClass: isFailed
        ? "health-detail-panel failed"
        : "health-detail-panel",
  ...
  syncObjectOptions(rows) {
    const objectNames = Array.from(
      new Set(
        (rows || [])
          .map((row) => row.Object_API_Name__c)
          .filter((value) => value)
      )
    ).sort();
    this.objectOptions = [
  ...
  matchesFilters(row) {
    if (
      this.filters.objectName !== "All" &&
      row.Object_API_Name__c !== this.filters.objectName
    ) {
      return false;
    }
    if (
      this.filters.status !== "All" &&
      row.Status__c !== this.filters.status
    ) {
      return false;
    }
    if (
      this.filters.dmlType !== "All" &&
      row.DML_Type__c !== this.filters.dmlType
    ) {
      return false;
    }
    return true;
  ...
  async fetchDebugLogBody(logId) {
    try {
      ...
      throw new Error(
        this.getRemoteErrorMessage(
          error,
          "We could not load the Apex debug log body. Please try again."
        )
      );
    }
  }
  ...
  getParserErrorMessage(error, fallback) {
    ...
    const safeParserMessage =
      /^(This file is larger than|This Apex debug log exceeds|The uploaded file is no longer available|The local log parser|Local log parsing|Another file is still being parsed|We could not read this file)/i;
  ...
  buildDebugSteps(events) {
    return (events || []).map((eventItem, index) => ({
      ...
      Error_Message__c:
        eventItem.severity === "error" ? eventItem.detail : null,
      ...
      Start_Time__c: null,
      End_Time__c: null,
      SOQL_Queries_Used__c:
        eventItem.type === "SOQL" && /\bBEGIN\b/i.test(eventItem.title || "")
          ? 1
          : 0,
      DML_Statements_Used__c:
        eventItem.type === "DML" && /started/i.test(eventItem.title) ? 1 : 0,
  ...
  async parseDebugLog(rawLog, dmlScope = null, signal = null) {
    ...
    const contextLines = (dmlScope?.contextEvents || []).map(
      (contextEvent) => ({
        line: `${contextEvent.timestampStr || "-"} (${contextEvent.startNanos || 0})|${contextEvent.type}|${contextEvent.detail || ""}`,
        isContext: true
      })
    );
    const logLines = getScopedLogLines(rawLog, dmlScope).map(
      ({ rawLine, lineIndex }) => ({
        line: rawLine,
        lineIndex,
        isContext: false
    ...
    for (let start = 0; start < lines.length; start += chunkSize) {
      ...
      for (const { line, lineIndex, isContext } of lines.slice(start, end)) {
        ...
        if (
          !isContext &&
          Number.isFinite(dmlScope?.startLine) &&
          lineIndex < dmlScope.startLine
        ) {
          continue;
        }
        if (
          !isContext &&
          Number.isFinite(dmlScope?.endLine) &&
          lineIndex > dmlScope.endLine
        ) {
          continue;
        }
        if (
          !isContext &&
          hasDmlScope &&
          !this.isWithinDmlScope(timestampNanos, dmlScope)
        ) {
          continue;
        }
        ...
        if (line.includes("FATAL_ERROR") || line.includes("EXCEPTION_THROWN")) {
          eventConfig = {
            ...
            title: this.cleanDebugMarker(marker),
            detail: this.cleanDebugDetail(details)
          };
          ...
          eventConfig = {
            ...
            title: this.cleanDebugMarker(marker),
            detail: this.extractSoqlDetail(details)
          };
        } else if (
          line.includes("VALIDATION_") ||
          /FIELD_CUSTOM_VALIDATION_EXCEPTION|REQUIRED_FIELD_MISSING/i.test(line)
        ) {
          const validation = marker.startsWith("VALIDATION_")
            ? this.parseValidationEventDetails(
                marker,
                details,
                lastValidationRuleName
              )
            : null;
          if (validation?.ruleName)
            lastValidationRuleName = validation.ruleName;
          ...
          const validationDetail = [
            ...
            validation?.detail ||
              (!validation ? this.cleanDebugDetail(details) : null)
          ]
            .filter(Boolean)
            .join(" | ");
          eventConfig = {
            ...
            detail: validationDetail || "Validation rule detail unavailable"
          };
          ...
          eventConfig = {
            ...
            title: this.cleanDebugMarker(marker),
            detail: this.extractCodeUnitDetail(details)
          };
        } else if (
  ...
  isWithinDmlScope(timestampNanos, dmlScope) {
    ...
    return (
      timestampNanos >= dmlScope.startNanos &&
      (!Number.isFinite(dmlScope.endNanos) ||
        timestampNanos <= dmlScope.endNanos)
    );
  }
  ...
  extractDmlEvent(marker, details, dmlScope = null) {
    ...
    const operation =
      this.extractDebugValue(details, "Op") || dmlScope?.operation || "DML";
    const objectApiName =
      this.extractDebugValue(details, "Type") ||
      dmlScope?.objectName ||
      "Unknown Object";
  ...
  extractSoqlDetail(details) {
    const entities =
      this.extractDebugValue(details, "Aggregations") ||
      this.extractDebugValue(details, "Rows");
  ...
  extractFlowDetail(details) {
    const flowName =
      details.match(/Interview Label:\s*([^|]+)/i)?.[1] ||
      details.match(/Flow:\s*([^|]+)/i)?.[1];
  ...
  parseValidationEventDetails(type, detail, previousRuleName = null) {
    ...
    const detailSegments = segments.filter(
