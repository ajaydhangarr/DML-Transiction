...
export default class DmlActionTree extends LightningElement {
  ...
  get hasAncestorContext() {
    return (
      Array.isArray(this.ancestorContext) && this.ancestorContext.length > 0
    );
  }
  ...
  get processedActions() {
    ...
    const maxDuration = Math.max(
      1,
      ...rows.map((row) => Number(row.durationMs) || 0)
    );
    this._processedCache = rows.map((row) => {
      const duration = Number(row.durationMs) || 0;
      const percentage =
        duration > 0
          ? Math.max(5, Math.round((duration / maxDuration) * 100))
          : 0;
      return {
        ...row,
        ...
        durationTone:
          duration >= 500
            ? "duration-tag duration-slow"
            : duration >= 100
              ? "duration-tag duration-warning"
              : "duration-tag",
  ...
  appendVisibleRows(actionList, rows, depth) {
    for (const action of actionList || []) {
      ...
      const isExpanded =
        this.expandedKeys.has(action.key) ||
        (!this.expandedKeys.has(`collapsed-${action.key}`) &&
